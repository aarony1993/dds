...
initializeApp({ projectId: "dds" });
const db = getFirestore();
const auth = getAuth();

// Feldpositionen & deren Modifikatoren
...
function getModifiersForPositionKey(posKey) {
  // Rückfall auf ST/MID/DEF, wenn Feldposition nur ST1, ZM2 etc. ist
...
// Utility: Weighted random (z.B. Duell, Schuss vs. Parade)
  ...
  async (event) => {
    ...
    if (dataBefore.status === "scheduled" && dataAfter.status === "live") {
      ...
      try {
        ...
        // Alle Spieler für beide Teams laden
        ...
        const tickRate = 100; // 100 ms = "Fast-Live" für Testzwecke. Im echten System eher 3000-6000 ms!
        const gameDuration = 10; // Minuten in ECHTZEIT (also ca. 100 Ticks für 90 "Ingame-Minuten")
        ...
        const gameInterval = setInterval(async () => {
          try {
            ...
            // Simulationslogik wie gehabt, aber Positionsboni immer nach positionKey (Feldposition!) zuweisen
            if (gameState.playState === 'open_play') {
                if(gameState.ballPosition === 'attack') {
                    ...
                    if (actionRoll > 0.5) {
                        // Dribbling (Angreifer vs. Verteidiger)
                        ...
                        // Pass (Angreifer zu zufälligem Mitspieler)
...
// ...restliche Funktionen wie vorher (acceptGameInvite, checkScheduledGames, cleanupOldInvites, addAdminRole, executeTransfer)...
...
// --- TRANSFER AUSFÜHREN ---
exports.executeTransfer = onCall(async (request) => {
