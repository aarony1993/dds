...
/*************************************************
 * Formation zuordnen (nur aufgestellte Spieler)
 *************************************************/
...
/*************************************************
 * Hauptfunktion: Spielzustand fortschreiben
 *************************************************/
function getNextGameState(
  ...
  let attacker = poolAtt.find(p => p.id === state.playerWithBall.id) || choosePlayer(poolAtt);
  ...
  let defender = choosePlayer(defList) || attacker;
  ...
  // Defensive Spieler dürfen nicht schießen/dribbeln in vorderster Linie
  ...
  switch(action) {
    ...
    case "header": {
      const kpr = choosePlayer(
        getPlayersByZone(opp,"defense")
          .filter(p=>normalizePosition(p.position)==="TW")
      ) || defender;
      const success = weightedRandomChance(
        0.23,
        attacker.strength,
      ...
      if (success) {
        ...
        const newStateAfterGoal = {
          ...
          attackTicks:      0,
          playerRatings,
          ratingEventsBuffer: appendRatingEvents(state.ratingEventsBuffer, tickDeltas, goalEvent.minute, 'header')
        };
    ...
    case "shoot": {
      ...
      if (success) {
        ...
        const newStateAfterGoal = {
          ...
          attackTicks:      0,
          playerRatings,
          ratingEventsBuffer: appendRatingEvents(state.ratingEventsBuffer, tickDeltas, goalEvent.minute, 'shoot')
        };
        return getKickoffState(home, away, homeTeam, awayTeam, newStateAfterGoal);
      } else {
        text       = describe("save", {
    ...
    case "duel": {
      const defList2 = getPlayersByZone(opp,curRel)
                        .filter(p=>normalizePosition(p.position)!=="TW");
    ...
    case "freekick": {
      ...
      if (success) {
        ...
        const newStateAfterGoal = {
          ...
          attackTicks:      0,
          playerRatings,
          ratingEventsBuffer: appendRatingEvents(state.ratingEventsBuffer, tickDeltas, goalEvent.minute, 'freekick')
        };
        return getKickoffState(home, away, homeTeam, awayTeam, newStateAfterGoal);
      } else {
        text       = describe("freekick_saved", {
          minute:     Math.round(currentMin),
          attacker:   formatName(attacker),
          goalkeeper: formatName(kpr2)
        });
